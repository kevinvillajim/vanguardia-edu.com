...
// Helper para mapear tipos de componentes del backend al builder
const mapBackendComponentType = (backendType: string): string => {
  ...
  return typeMap[backendType.toLowerCase()] || 'reading';
};
...
export const EditCoursePage: React.FC = () => {
  ...
  const autoSave = useHybridAutoSave({
    ...
    enabled: !!currentCourseId, // Activar cuando tenemos ID de curso
    onSaveSuccess: () => {
  ...
  // Función para cargar el curso existente
  const loadCourse = async () => {
    if (!courseId) {
      setLoadError('ID de curso no proporcionado');
    ...
    try {
      ...
      const result = await courseService.getTeacherCourse(parseInt(courseId));
      
      if (result.success && result.course) {
        const course = result.course;
        // Mapear los datos del curso al formato del formulario
        setCourseData({
          title: course.title || '',
          description: course.description || '',
          categoryId: course.category?.id || null,
          difficulty_level: course.difficulty_level || CourseDifficulty.BEGINNER,
        ...
        // Establecer ID del curso para edición
        if (course.id) {
          setCurrentCourseId(course.id);

          // Debug: Ver estructura recibida
          logger.info('📚 EditCoursePage: Curso completo recibido:', course);
          logger.info('📚 EditCoursePage: Estructura del curso:', {
            hasUnits: !!course.units,
          ...
          // Inicializar builderData con la estructura correcta
          if (course.units && course.units.length > 0) {
            // Las units del backend contienen modules que contienen components
            // Necesitamos aplanar esta estructura para el builder
            const flattenedModules = course.units.map((unit: any, unitIndex: number) => {
              logger.info(`📚 Procesando unit #${unitIndex + 1}:`, unit);
              
              // Recolectar todos los componentes de todos los módulos de esta unit
              const allComponents: any[] = [];
              
              unit.modules?.forEach((module: any) => {
                module.components?.forEach((comp: any, compIndex: number) => {
                  let parsedContent = comp.content;
                  try {
                    if (typeof comp.content === 'string' && comp.content) {
                      parsedContent = JSON.parse(comp.content);
                    }
                  } catch (e) {
                    logger.warn('⚠️ No se pudo parsear el content del componente:', comp.content);
                    parsedContent = {};
                  }
                  
                  // Mapear metadata si existe
                  let parsedMetadata = comp.metadata;
                  try {
                    if (typeof comp.metadata === 'string' && comp.metadata) {
                      parsedMetadata = JSON.parse(comp.metadata);
                    }
                  } catch (e) {
                    parsedMetadata = {};
                  }
                  ...
                  const mappedComponent = {
                    id: `${unit.id}-${module.id}-${comp.id}`,
                    backendComponentId: comp.id, // Este es el ID real del backend
                    type: mapBackendComponentType(comp.type),
                    title: comp.title,
                    content: parsedContent,
                    fileUrl: comp.file_url,
                    duration: comp.duration || parsedMetadata?.duration,
                    order: comp.order || compIndex,
                    isMandatory: comp.is_mandatory !== false,
                    metadata: parsedMetadata
                  };
                  
                  logger.info(`  📄 Component: ${mappedComponent.title} (${mappedComponent.type}) - Backend ID: ${comp.id}`, mappedComponent);
                  allComponents.push(mappedComponent);
                });
              });
              
              const mappedModule = {
                id: String(unit.id), // Convertir a string para el builder
                title: unit.title,
                subtitle: unit.description,
                description: unit.description,
                bannerImg: unit.banner_image,
                order: unitIndex + 1,
                components: allComponents.sort((a, b) => a.order - b.order), // Ordenar por order
                backendUnitId: unit.id,
                backendModuleId: unit.modules?.[0]?.id, // Tomamos el primer módulo de la unit
                enableSmartProgress: false
              };
              
              logger.info(`📚 Módulo procesado con ${allComponents.length} componentes:`, mappedModule);
              return mappedModule;
            });

            logger.info('📚 Módulos procesados para el builder:', flattenedModules);
            ...
            setBuilderData({
              ...
              activities: [] // Por ahora sin activities
            });
          } else if (course.modules && course.modules.length > 0) {
            // Fallback: si no hay units pero sí modules directos (compatibilidad)
            setBuilderData({
              id: course.id,
              title: course.title,
              description: course.description,
        ...
        logger.success('✅ Curso cargado para edición:', course.title);
      } else {
        setLoadError(result.error || 'Curso no encontrado');
        logger.warn('⚠️ Error cargando curso:', result.error);
      }
    } catch (error: any) {
      logger.error('❌ Error cargando curso:', error);
      setLoadError(error.message || 'Error al cargar el curso');
    } finally {
      setIsLoadingCourse(false);
    }
  };
  ...
  }, [currentCourseId, courseData, updateCourseBasicDataDebounced]);
  ...
  useEffect(() => {
    if (currentCourseId && (courseData.title || builderData)) {
      const draftData = {
        courseData,
        builderData,
  ...
  }, [courseData, builderData, isDirty, currentCourseId, updateCourseBasicData]);
  ...
  const saveDraft = async () => {
    if (!currentCourseId) {
      logger.warn('⚠️ No se puede guardar: ID de curso no encontrado');
      return;
    }
  ...
  const handleBuilderDataChange = (data: any) => {
    setBuilderData(data);
    setIsDirty(true);
  };
...
- Cancelar: No crear ninguna categoría`;
  ...
  const saveSettings = async () => {
    if (!currentCourseId) {
      logger.error('❌ No se puede guardar: ID de curso no encontrado');
    ...
    setSaving(true);
    try {
      // En modo edición, SIEMPRE actualizar el curso existente
      const { courseService } = await import('../../services/courseService');
      const result = await courseService.updateCourse(currentCourseId, courseData);
      
      if (result.success) {
        logger.success('✅ Configuración del curso actualizada exitosamente');
        setIsDirty(false);
      } else {
        throw new Error(result.message || 'Error actualizando curso');
      }
    } catch (error) {
      logger.error('❌ Error actualizando configuración:', error);
      alert('Error al actualizar la configuración del curso');
    } finally {
      setSaving(false);
    }
  };
  ...
  const isTabEnabled = (tabId: TabId): boolean => {
    // En modo edición, todas las pestañas están disponibles
    return true;
  };
  ...
  const handleSaveAndExitNew = async (): Promise<void> => {
    ...
    try {
      if (currentCourseId) {
        // Forzar guardado de cambios del curso
        await saveDraft();
        logger.success('✅ Cambios del curso guardados');
      } else {
        logger.warn('⚠️ No hay curso para guardar');
      }
  ...
  const renderTabContent = () => {
    // Log para debug
    if (activeTab === 'builder' && builderData) {
      logger.info('📚 EditCoursePage: Pasando builderData a TeacherCourseBuilder:', builderData);
    }
      ...
      case 'builder':
        return (
          <TeacherCourseBuilder 
            ...
            initialBuilderData={builderData}
            key={currentCourseId} // Forzar re-renderizado cuando cambia el ID
          />
        );
      case 'preview':
        return (
      ...
      <DashboardLayout>
        <div className="flex justify-center items-center min-h-screen">
          <div className="text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">Cargando curso...</h3>
            <p className="text-gray-500">Obteniendo información del curso</p>
          </div>
        </div>
      </DashboardLayout>
    );
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  ...
                  {currentCourseId && autoSave.isActive && (
                    <>
                      <span>•</span>
                  ...
                  {(!currentCourseId || !autoSave.isActive) && saving && (
                    <>
                      <span>•</span>
                      <span className="text-primary-600">Guardando...</span>
                    </>
                  )}
                  {(!currentCourseId || !autoSave.isActive) && isDirty && !saving && (
                    <>
                      <span>•</span>
            ...
            <div className="flex items-center gap-3">
              {/* Botón Publicar - solo visible cuando hay curso válido */}
              {currentCourseId && (
                <Button
                  variant="success"
                  size="sm"
